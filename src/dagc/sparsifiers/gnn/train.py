...
from dagc.data.read_graphs import generate_seed_set
from dagc.sparsifiers.gnn.gnn_sparsifier import GNNSparsifier
from dagc.sparsifiers.gnn.losses import ic_preservation_loss, random_walk_preservation_loss
...
#             # choose seeds (e.g., high-degree or random)
#             seed_set = generate_seed_set(G, k=10, strategy="high_degree")

#             optimizer.zero_grad()
#             out = model(x, edge_index)

#             loss, comps = ic_preservation_loss(
#                 G=G,
#                 edge_probs=out.edge_probs,
#                 edge_index=edge_index,
#                 seed_nodes=seed_set,
#                 base_p=0.1,       # same as used in evaluation / IC
#                 keep_ratio=keep_ratio,
#                 lambda_sparsity=lambda_sparsity,
#                 num_steps=num_steps,          # IC steps (1–3 is reasonable)
#                 device=dev,
#                 return_components=True,
#             )
...
#         denom = max(1, num_graphs)
#         train_loss = train_total_sum / denom
#         train_ic_mse_loss = train_ic_mse_sum / denom
#         # train_rw_loss = train_rw_sum / denom
#         train_sparsity_loss = train_sparsity_sum / denom
...
#         # history["val_sparsity_loss"].append(val_sparsity_loss)
#         history["train_avg_prob"].append(avg_prob_sum / num_graphs)
#         history["train_ic_loss"].append(train_ic_mse_loss)


#         if epoch % print_every == 0:
#             print(
#                 f"[Epoch {epoch}/{num_epochs}] "
#                 f"train_loss = {train_loss:.6f} (ic_mse={train_ic_mse_loss:.4f}, sparse={train_sparsity_loss:.4f}) "
#                 # f"val_loss = {val_loss:.6f} (rw={val_rw_loss:.4f}, sparse={val_sparsity_loss:.4f})"
#             )
